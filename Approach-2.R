...
# Drop id column
bc_data <- subset (bc_data, select = -c(id))
...
round(prop.table(table(bc_data$diagnosis)), 2)
...
boxplot(bc_data[-1])
ggpairs(bc_data[1:10], aes(colour = bc_data$diagnosis, alpha = 0.6))
ggpairs(bc_data[11:20], aes(colour = bc_data$diagnosis, alpha = 0.6))
ggpairs(bc_data[21:31], aes(colour = bc_data$diagnosis, alpha = 0.6))

# delete outliers using quan method, MVN library
outliers <- mvn(data = subset (bc_data, select = -c(diagnosis)), multivariateOutlierMethod = "quan", showOutliers = TRUE)
bc_data_clean <- bc_data[-as.numeric(outliers$multivariateOutliers$Observation),]

dim(bc_data_clean)
str(bc_data_clean)

# Quick look at variables distribution after removing outliers
...
# The frequency distribution of benign and malignant groups after removing outliers
...
df_corr <- cor(subset (bc_data_clean, select = -c(diagnosis)))
...
bc_data_cor <- bc_data_clean %>% select(-findCorrelation(df_corr, cutoff = 0.9))
dim(bc_data_cor)
str(bc_data_cor)
